import assets.scripts.package as pip
...
def create_kinematic_ball(space, pos):
...
def text(font_render, text):
    ...
    text_rect = text_render.get_rect(center = (960 / 2, 57))
    text_shadow_render = font_render.render(text, True, (0, 0, 0))
    text_shadow_rect = text_render.get_rect(center = (960 / 2, 60))
...
circles = []
circles.append(create_static(space, (640, 384)))
circles.append(create_static(space, (320, 320)))

goals = []
goals.append(create_kinematic_ball(space, (480, 480)))
...
while True:
    ...
    if new_level == False:
        ...
        if hole_timer >= hole_max_timer:
            ...
            circles = []
            if level == 0:
                circles.append(create_static(space, (10, 6)))
                circles.append(create_static(space, (5, 5)))
            elif level == 1:
                circles.append(create_static(space, (8, 10)))
                circles.append(create_static(space, (7, 5)))
            elif level == 2:
                circles.append(create_static(space, (12, 6)))
                circles.append(create_static(space, (3, 6)))
            stroke = 0
            level += 1
            new_level = False
            ...
            if hole_max_timer / hole_timer >= 3:
                body.position = (480 + (hole_timer / 11) + random.randint(-3, 3), 480 + (hole_timer / 11) + random.randint(-3, 3))
            elif 3 > hole_max_timer / hole_timer >= 2:
                body.position = (480 + (hole_timer / 12) + random.randint(-2, 2), 480 + (hole_timer / 12) + random.randint(-2, 2))
            elif 2 > hole_max_timer / hole_timer >= 1:
                body.position = (480 + (hole_timer / 12.825) + random.randint(-1, 1), 480 + (hole_timer / 12.825) + random.randint(-1, 1))
            if size <= 86:
